...
> **v6-full 勘误（2026-06-26 · D11/D13）**：内核 **10 模块**（含 `platform_registry` · ADR-008）；物理路径 `src/server/os_core/` · import `os_core.*`；迁移 `src/server/db/migrations/` + 根 `alembic.ini`（**无** `alembic/` 目录）。下文若写「9 模块」为 2026-06-16 历史口径。
...
├─────────────────────────────────────────────────────────────────┤
│  应用层                                                          │
│    · server/api          后端 HTTP 服务（FastAPI，唯一生产 deployable）│
│    · src/apps/web-admin    PC 管理端（Graph freeze、规则、运维）       │
│    · src/apps/h5-worker    移动端壳（钉钉/企微 H5，调 API）            │
├─────────────────────────────────────────────────────────────────┤
...
**仓库结构（2026-06-26 v6-full 勘误）：**

```text
FactoryOS/
├── src/
│   ├── server/
│   │   ├── api/                 # 1 个后端 deployable（import: server.api）
│   │   ├── os_core/             # Core v1.0 冻结域（import: os_core.*）
│   │   ├── db/migrations/
│   │   └── edge-agent/
│   ├── apps/
...
│   └── tests/                   # AC-BASE-001 等
└── docs/文档/                   # Schema、OpenAPI v1.1.1 契约
```
...
flowchart LR
    ...
    API[server/api] --> GS & RE & ES & AS & AO & LS & MG
    ES --> CS
    ES --> AS
    AO -.->|"仅输出计划"| ES
```
...
| **Studio** | 六步实施向导 | `src/apps/web-admin`（`/studio/*`） | P1 MVP |
