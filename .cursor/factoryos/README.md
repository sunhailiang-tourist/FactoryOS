# .cursor/factoryos · SH-步步流 v2 认知精髓

Agent 日常协作 **不依赖 docs/**；机器契约见 `contracts/`。

| 文件 | 内容 |
|------|------|
| [UI-FIRST-CONFIG-PRINCIPLE.md](./UI-FIRST-CONFIG-PRINCIPLE.md) | **产品宪法** · 管理台主路径 |
| [INTEGRATION-CHAIN.md](./INTEGRATION-CHAIN.md) | 接入/扩展 · Studio |
| [PRE-DEV-CHAIN.md](./PRE-DEV-CHAIN.md) | **开发前全链路** · 维护地图 |
| [INDEX.md](./INDEX.md) | 总索引 |
| [ACTIVATION.md](./ACTIVATION.md) | 一次性激活清单 |
| [SH-步步流.md](./SH-步步流.md) | v2 工作流 |
| [GATES.md](./GATES.md) | L1 关键词 |
| [STEP0.md](./STEP0.md) | Dev 全量理解 |
| [DEV-GATES.md](./DEV-GATES.md) | Dev Gate 1–4 |
| [TEST-GATES.md](./TEST-GATES.md) | Test Gate A–G |
| [VERIFY-GATES.md](./VERIFY-GATES.md) | Verify 独立审阅 |
| [HARNESS-SCRIPTS.md](./HARNESS-SCRIPTS.md) | gate / harness 节点 |
| [REDLINES.md](./REDLINES.md) | R-01–R-11 |
| [MODULE-MAP.md](./MODULE-MAP.md) | 九模块 |
| [AC-P0-INDEX.md](./AC-P0-INDEX.md) | 52 P0 |
| [templates/](./templates/) | 落盘模板 |
| [../docs-baseline/BASELINE.md](../docs-baseline/BASELINE.md) | docs 认知基线 |
