...
研发轨（PRE-DEV-CHAIN）  平台团队写 os_core/api/studio · SH-步步流
```
...
| 类型 | 路径 |
|------|------|
| Gate 状态机 | `src/integration/tools/guide/flows.json` |
| OpenAPI | `contracts/openapi/工厂操作系统-v1.1.yaml` |
| Studio 路由 | `apps/web-admin` `/studio/*`（规格见 docs） |
| 运行时配置 | PostgreSQL（非 Git YAML 为主） |
