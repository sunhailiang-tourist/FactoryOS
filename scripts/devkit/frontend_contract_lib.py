...
ROUTE_PERMISSIONS = re.compile(r'permissions:\s*\["([^"]+)"\]')
...
def parse_route_entry(reg_text: str) -> dict[str, str | bool]:
  ...
  return {
    ...
    "permission": ROUTE_PERMISSIONS.search(reg_text).group(1) if ROUTE_PERMISSIONS.search(reg_text) else "",
  }
