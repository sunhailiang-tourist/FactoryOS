...
| Step3 规模预埋 | `src/server/db/migrations/` · `tenant_registry.py` · `outbox.py` | S-01～S-04 四表 + seed + 端口 |
...
- SQLite 测试用 `cache=shared` + Alembic connection 注入（见 `src/server/db/migrations/env.py`）
- httpx 0.28 → 集成测试改用 FastAPI `TestClient`
- `src/server/db/migrations/` 为长期 migration 目录，非临时脚手架
